...
<!-- /TinyMCE -->
		...
		<?php 	
				...
				$promo = mysql_query("SELECT * FROM carapesan WHERE id_carapesan='1'");
				$sql=mysql_fetch_array($promo);
				?>
		
		<article class="module width_full">
			<header><h3>Edit Cara Pesan</h3></header>
			<form method='POST' enctype='multipart/form-data' action='<?php echo "$aksi"; ?>?module=carapesan&act=update'>
				<input type='hidden' name='id' value='<?php echo "$sql[id_carapesan]" ?>'>
				<div class="module_content">
							<textarea name="isi" rows="12"><?php echo "$sql[isi]" ?></textarea>
						<div class="clear"></div>
				</div>
			<footer>
