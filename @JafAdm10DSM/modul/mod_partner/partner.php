...
<!-- /TinyMCE -->
		...
		<article class="module width_full">
			<header><h3 class="tabs_involved">Daftar Patner</h3>
				
				<input style="float:right; margin-top:5px;margin-right:15px;" type='button'  class='tombol' value='Insert New' onclick="location.href='?module=partner&act=insertnew'">
		...
		<div class="tab_container">
			...
			<?php 	
				$no=1;
				$partner = mysql_query("SELECT * FROM partner ORDER BY id_partner");
				while($m=mysql_fetch_array($partner)){
				
				?>
				<tr>  
    				<td align="center"><?php echo"$no" ?></td>
    				<td><img width="100px" height="50px" src="../images/leasing/<?php echo"$m[image]" ?>"></td> 
    				<td><?php echo"$m[leasing]" ?></td> 
    				<td><?php echo"$m[link]" ?></td> 
    				<td align="center"><a href="<?php echo"?module=partner&act=edit&id=$m[id_partner]";?>">
					<input type="image" src="images/icn_edit.png" title="Edit"></a></td> 
					<td align="center"><a href="<?php echo"$aksi?module=partner&act=hapus&id=$m[id_partner]";?>" 
		...
		<article class="module width_full">
			<header><h3>Tambahkan Patner</h3></header>
			<form method='POST' enctype='multipart/form-data' action='<?php echo "$aksi"; ?>?module=partner&act=insertnew'>
				<div class="module_content">
						
						<table>
							
							<tr><td style="width:20%;"><label>Nama Patner</label></td> <td style="width:50%; margin-bottom:8px;">
		...
		<?php break; 
		case"edit":
			$partner = mysql_query("SELECT * FROM partner WHERE id_partner='$_GET[id]'");
			$r=mysql_fetch_array($partner);
			
		?>
		
		<article class="module width_full">
			<header><h3>Edit Patner</h3></header>
			<form method='POST' enctype='multipart/form-data' action='<?php echo "$aksi"; ?>?module=partner&act=update'>
				<input type='hidden' name='id' value='<?php echo"$r[id_partner]" ?>'>
				<div class="module_content">
						<table>
							
							<tr><td style="width:20%;"><label>Nama Patner</label></td> <td style="width:50%; margin-bottom:8px;">
							<input style="width:100%; margin-bottom:8px;" name="nama" type="text" value="<?php echo"$r[leasing]" ?>"></td></tr>
							<tr><td style="width:20%;"><label>Link</label></td> <td style="width:50%; margin-bottom:8px;">
							<input style="width:100%; margin-bottom:8px;" name="isi" type="text" value="<?php echo"$r[link]" ?>"></td></tr>
						</table>
						<fieldset style="float:left; width:30%; margin-right: 3%;"> <!-- to make two field float next to one another, adjust values accordingly -->
							&nbsp;&nbsp;<img width="270px" src="../images/leasing/<?php echo"$r[image]" ?>">
